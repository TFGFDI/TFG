...
<section id="derecho_general" class="bloqueRedondo bloqueSombra">
	...
	<div id="nuevaNoticia" style="width:120px; float:right; margin:10px 10px 0px 0px; ">
		<input type="button" name="b_nuevaNoticia" value="Nueva Noticia" onclick="nuevaNoticia()" id="b_nuevaNoticia">
	</div>
	<div id="buscador" class="bloqueRedondo  bloqueSombra" style="margin-top:60px; border:1px solid silver;">
		<div id="buscadorIzq">
			<form name="buscador" method="get" action="noticias.php" id="buscador">
				<div class="divCampo">
			...
			<table>
				<thead>
					<tr>
						...
						<th onclick="orden('descripcion','<?php echo $orden?>');" style="cursor:pointer"><span>Descripción</span>
							<?php 
								if($filtro=="apellidos"){
									if($orden=="DESC"){ ?>
										<img src="../imagenes/flecha-abajo.png">
				...
				<?php
					...
					$filasPag = $noticia->getNoticisPaginacion($buscador,$activo,$fecha,$filtro,$orden,$itemsInicio,$numer_reg);
				...
				?>
					<tr <?php if($i%2==0){?>class="alt"<?php }else{?>class="impar"<?php }?> >
						...
						<td><a class="ifancybox" href="visualizarNoticia.php?id=<?php echo $rowEmp['idNoticia']?>"><?php echo $rowEmp['titulo']?></a></td>
						...
						<td style="cursor:pointer;text-align:center" id="<?php echo $rowEmp['idNoticia']?>" onclick="editar(this.id)"><img src="../imagenes/lapiz.gif"></td>
						<td style="cursor:pointer;text-align:center" id="<?php echo $rowEmp['idNoticia']?>" onclick="eliminar(this.id)"><img src="../imagenes/eliminar.png" style="width:15px;"></td>
					</tr>
					<?php 
					$i++;
					}?>
